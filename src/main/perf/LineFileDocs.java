  ...
  public static final class DocState {
    ...
    DocState(boolean storeBody, boolean tvsBody, boolean bodyPostingsOffsets, boolean addDVFields) {
      ...
      if (addDVFields) {
        ...
        doc.add(lastModNDV); 
        lastModLP = new LongPoint("lastModNDV", -1); //point field must have the same name as DV field
        doc.add(lastModLP);
        ...
        dayOfYearDV = new NumericDocValuesField("dayOfYearNumericDV", 0);
      	doc.add(dayOfYearDV);
        dayOfYearIP = new IntPoint("dayOfYearNumericDV", 0); //point field must have the same name as DV field
        doc.add(dayOfYearIP);
      } else {
      	titleDV = null;
      	lastModNDV = null;
